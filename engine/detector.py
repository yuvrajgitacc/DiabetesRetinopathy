...
Primary: RishiSwethan's EfficientNet-B3 (PyTorch, 41MB, 5-class, 300x300)
Fallback: Tanwar-12's CNN (TensorFlow, 0.4MB, regression, 64x64)

The EfficientNet-B3 model provides significantly better accuracy
(~56.8% val acc on 5-class DR) vs the tiny CNN.
"""
import numpy as np
import os
import json
...
def _load_pytorch_model():
    """Load RishiSwethan's EfficientNet-B3 model."""
    ...
    try:
        ...
        print("[MODEL] Loading EfficientNet-B3 (RishiSwethan) from {}...".format(PYTORCH_MODEL_PATH))
        ...
        _active_model_type = 'pytorch'
        print("[OK] EfficientNet-B3 loaded! (41MB, 5-class, 300x300 input)")
        return model
...
def _predict_pytorch(image, model):
    ...
    return {
        ...
        "_model": "EfficientNet-B3 (RishiSwethan)",
    }
